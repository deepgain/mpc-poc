...
> **Uwaga:** Ablacje A1/A2/A3 były przeprowadzone na modelu **przed Wariantem 2** — bez strength anchor features (`strength_feature_dim=0`). Model nie rozróżniał wtedy użytkowników o różnej sile (znana dziura opisana w `VARIANT2_SUMMARY.md`). Wyniki nie są porównywalne bezpośrednio z finalnym modelem Wariant 2 (100ep, RMSE 0.841). Checkpointy `deepgain_ablation_*.pt` zachowane jako punkt odniesienia dla architektury pre-V2.
